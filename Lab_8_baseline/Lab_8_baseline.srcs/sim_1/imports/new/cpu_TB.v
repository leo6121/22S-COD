...
module cpu_TB();
	...
	cpu UUT (CLK, reset_n, dma_start_int, BR, dma_end_int, BG, cmd, i_readM, i_writeM, i_address, i_data, d_readM, d_writeM, d_address, d_data, num_inst, output_port, is_halted);
	Memory NUUT(!CLK, reset_n, BG, i_readM, i_writeM, i_address, i_data, d_readM, WR, addr_bus, data_bus);	

	assign d_data = (d_writeM && !d_readM) ? 64'bz : data_bus;

	assign data_bus = (BG) ? dma_data : 
					  (d_writeM && !d_readM) ? d_data : 64'bz;
	assign addr_bus = (BG) ? dma_addr : d_address;
	assign WR = (BG) ? dma_READ : d_writeM;
