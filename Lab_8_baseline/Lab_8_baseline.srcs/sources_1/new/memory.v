...
module Memory(CLK, reset_n, BG, i_readM, i_writeM, i_address, i_data, d_readM, WR, addr_bus, data_bus);
	input CLK;
	wire CLK;
	input reset_n;
	wire reset_n;
	...
	assign data_bus = (d_readM)?d_outputData:64'bz;

	assign i_targetaddr = {i_address[15:2], 2'b0};
	assign d_targetaddr = {addr_bus[15:2], 2'b0};
	...
	always@(posedge CLK) begin
		if(!reset_n)
			...
			begin
				...
				if(WR && !BG && d_mem_counter == 3'd3)memory[addr_bus] <= data_bus[63:48];
				if(WR && BG && d_mem_counter == 3'd3) begin
					memory[addr_bus] <= data_bus[63:48];
					memory[addr_bus+1] <= data_bus[47:32];
					memory[addr_bus+2] <= data_bus[31:16];
					memory[addr_bus+3] <= data_bus[15:0];
				end
			end
	end
endmodule